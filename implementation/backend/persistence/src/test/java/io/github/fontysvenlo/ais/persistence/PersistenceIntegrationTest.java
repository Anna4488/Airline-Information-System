 ...
 * place to start.
 * You will need to add some database setup and can prefill the database with
 * some data before a test.
 * 
 ...
 * postgres.start();
 * config = new DBConfig("test", postgres.getHost(),
 * postgres.getFirstMappedPort(), "test", "ais", "test", "test");
 * }
 * 
 * @AfterAll
 ...
 * persistenceAPI.getCustomerRepository();
 * 
 * // Act
 * CustomerRepository.add(new CustomerData(0, "John", "Doe", LocalDate.of(2025,
 * 1, 1)));
 * List<CustomerData> customers = CustomerRepository.getAll();
 * 
 * // Assert
 * // Note: this assumes that the database was empty before the test.
 * // The intention of the test is to make sure that if we store a customer, we
 * can retrieve it.
